...
// Member functions
double Complex::Mag2() const { // Mag squared
...
void Complex::Print() const {
  if (NaN) {
      ...
      if (imag == 0) { // just real part with no parens
          cout << real;
        }
      else {
          cout << '(' << real << "," << imag << ')';
...
std::ostream& operator << (std::ostream &os, const Complex& c) { // Provided by the Prof. Riley
  if (c.NaN) {
      os << "NaN";
    }
  else {
      if (c.imag == 0) { // just real part with no parens
          os << c.real;
        }
      else {
          os << '(' << c.real << "," << c.imag << ')';
